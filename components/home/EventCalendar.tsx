                ...
                return (
                  <div key={c.iso}
                    ...
                    <div className="flex flex-col gap-0.5">
                      {shown.map((e, i) => (
                        <Link key={i} href={`/calendar?date=${c.iso}`} title={`${e.title} · ${CATEGORY_LABEL[e.category]}${e.desc ? ` · ${e.desc}` : ''}`}
                          className={`block text-[10px] leading-tight font-semibold truncate rounded border px-1 py-0.5 hover:brightness-105 ${chipCls(e)}`}>
                          {e.title}
                        </Link>
                      ))}
                      {more > 0 && <Link href={`/calendar?date=${c.iso}`} className="text-[9.5px] text-[var(--text-muted)] font-semibold pl-0.5 hover:text-[var(--accent)]">+{more}건</Link>}
                    </div>
                  </div>
                );
              })}
