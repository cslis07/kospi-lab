...
**전체 점검 1차 완료 · 미결 항목 있음** — 마지막 커밋 `24ee41c`. 배포·검증 완료(테스트 15/15, tsc 0, build OK, 게이트 401 실측).
...
### 🔴 결정 대기 — 엔진 감사에서 나온 실거래 영향 버그 (미착수)
전부 라이브 데이터로 재현된 것들. 상세는 §7-A.
- **C-1 분할매수 3차 지정가가 자기 손절선 밖** — 라이브 4코인 중 3개 재현. 화면의 평단·손절손실이 실제와 다름(BTC 실측: 표시 -10.00 USDT vs 실제 -3.84)
- **H-4 권장 사이징이 시드보다 큰 증거금 요구** — BTC 손절폭 0.122%에서 보수 4배 증거금이 시드의 205%. 검증·경고 없음
- **H-3 피보나치가 target2를 target1보다 나쁘게 덮어씀** (롱·숏 양쪽, 0.2R까지 하락)
- **레버리지 "3요소"가 실제로는 1요소** — `signalGrade`가 score가 아니라 `entryOk`의 별칭(`coinAnalysis.ts:631`). 실사용 구간에서 나머지 2요소는 캡에 포화되어 상수. score 22와 55가 같은 4/8배가 나오는 원인
- **H-5 주식 백테스트가 원리적으로 신호 0건** — `extras={}`라 `strongSupply`가 항상 false → `entryOk` 항상 false. 프로덕션 실측 `signals:0`
- **H-6 `stockAnalysis` target1이 `Infinity`** — `Math.min(...[])`가 truthy라 `||` 폴백 불발 → UI "∞원", 익절 자동판정 불능
- **H-2 백테스트 lookahead** — 1H 지표가 평균 27.6분(최대 50분) 미래를 봄. 성적표가 실제보다 낙관적
- 그 외 M급 7건(숏 손절률 부호 "−" 오표기, `confidence`가 차단 게이트 미반영, 펀딩 회피가 정산 직후 통과, `ema200` 시드 미사용 등)

### 이번 세션에 고친 것
① 보안 3건(키 폴백 제거·게이트 확대·죽은 `/api/kis/token` 삭제) ② 지표 계산 2건(CPI YoY 13개월→12개월 3.73%→3.46%, 가계부채 2025Q1→2026Q1·QoQ 복구) ③ 기록 안전망(백업이 접두사 전체를 담도록 v2, `/virtual` 내비 노출, 삭제 확인) ④ 분석 실패 가시화(`lib/fetcher.ts` + 두 페이지 에러 배너).
