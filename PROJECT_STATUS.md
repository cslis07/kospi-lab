...
**깨끗한 상태** — 워킹트리 비어 있음(루트에 핸드오프 PDF 1개만 미추적, 커밋 대상 아님). `main`이 `origin/main`과 동기. 게이트 전부 통과(테스트 75/75 · tsc 0 · build OK).
...
5. 🆕 **실행 가능 판정(Go/No-Go)** `lib/tradeGate.ts` — "사도 되는가"에 답하는 정직한 방법.
   방향은 못 맞히므로(측정됨) **"이 크기로 걸어도 감당되나"**만 단언한다. 7개 검사 →
   GO/RESIZE/NO + 실패 시 권장 상한(배율·노션·허용손실). `/coin-analysis` 리스크 패널에 연결.
   ⚠ GO 는 "오른다"가 아니라 "틀려도 버틴다" — 이 구분이 이 기능의 전부다.
