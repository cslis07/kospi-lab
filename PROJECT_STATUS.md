...
> **규모:** API 라우트 40 · 페이지 25 · lib 28 · hooks 11 · components 19 · scripts 9 · tests 1(42케이스)
...
- **Bitget 키에 선물 읽기 권한 추가** — `/api/bitget/positions`가 `40014: need future pos read`로 실패한다(프로덕션 실측). 현재 키가 현물 전용. Bitget API 관리에서 **선물 포지션 읽기(Futures/Position - Read)**를 추가(읽기 전용 유지)하면 `/bitget`에 실제 청산가·미실현손익이 표시된다. 라우트·서명은 정상.
...
2026-07-27 감사 항목 중 아직 안 고친 것. 진입 판정 자체를 오염시키는 건 없다.
...
**감사 항목 처리 현황** — H-2 lookahead·vwap 기준일·M-6 ema200 시드(2026-08-07 1차), **M-1 수급 전부 0·M-2 숏 손절 부호·M-4 confidence 게이트·M-5 펀딩 정산 직후(3차)** 완료. 저널 레버리지 실사용값·저널 100→1000·Bitget 선물 연동도 3차에서 처리.
...
**매매일지 성적표 `/journal`** — 이 앱의 정직한 핵심. 코인·주식 저널을 시간창별(7/30일/전체) 승률·기대값·R 분포·미실현손익·미청산 비율(규율 신호)로 실측. `lib/journalStats.ts` 순수 함수(테스트 고정).

**Bitget 선물 포지션 `/bitget`** — 현물 + **USDT 선물 포지션**(mix API). 실제 평단·레버리지·미실현손익·청산가·청산까지 거리(%), 15% 미만 붉은 경고. 읽기 전용, 15초 폴링.
...
### 개선 여지
- [ ] **M-7/M-8** — §0 잔여 목록. 표시·정확도 계열이고 진입 판정을 오염시키지 않아 급하지 않다
