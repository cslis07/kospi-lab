...
### 분석 (Analysis)
- **국내주식 분석** `/stock-analysis` — 🆕 코인 분석과 같은 스타일의 종목 매수판단. 종목코드 입력/인기종목 프리셋, 일봉 추세(EMA·RSI·MACD·구조)+**투자자 수급(개인·외국인·기관 10일 순매수 + 외국인 보유율 추세)**+밸류에이션(PER·PBR·ROE·재무등급) 룰 엔진 → 매수우위/중립/비중축소 + 손절·목표(폭 3~15% 제한). "지금 왜 오르나/내리나" 드라이버, 일봉 캔들차트(지지저항·피보나치·매매레벨), 룰 엔진 백테스트, 투자자 수급 10일 테이블, 종목뉴스 호재/악재 태깅, AI 브리핑. 서울 리전(icn1) 고정
