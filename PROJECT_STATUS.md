...
### 분석 (Analysis)
- **국내주식 분석** `/stock-analysis` — 🆕 코인 분석과 같은 스타일의 종목 매수판단. **메릴린치 CIO 업종의견(2026.6.29, 종목→GICS 11업종 매핑 후 비중확대/중립/비중축소 반영)** + **필수 경제지표 패널 실측값 자동수집**(미국 CPI YoY=BLS 키없이, 가계신용·주택가격지수=한국은행 ECOS, 엔화·원달러 라이브, 반도체수출=관세청 CUSTOMS_API_KEY 필요·없으면 체크필요, 6h 캐시) 상시 참고. **종목명 자동완성 검색**(/api/stock-search, 우선주 후순위)/인기종목 프리셋, 일봉 추세(EMA·RSI·MACD·구조)+**투자자 수급(개인·외국인·기관 10일 순매수 + 외국인 보유율 추세)**+밸류에이션(PER·PBR·ROE·재무등급)+**DART 공시 자동분류·정부정책/테마 키워드 감지** 룰 엔진 → 매수우위/중립/비중축소 + 손절·목표(폭 3~15% 제한). "지금 왜 오르나/내리나" 드라이버, 일봉 캔들차트(지지저항·피보나치·매매레벨), 룰 엔진 백테스트, 투자자 수급 10일 테이블, 종목뉴스+DART 공시 호재/악재 태깅, AI 브리핑(【지금 왜 움직이나/수급/공시·정책/종합판단】). 서울 리전(icn1) 고정, DART_API_KEY 필요(없으면 공시만 생략)
