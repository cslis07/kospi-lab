...
**🌱 펀딩 극단 = 유일하게 신호가 보인 영역 (확립 아님, 후보)** — `scripts/backtest-funding.ts`

|펀딩| 0.01~0.03% 구간에서 붐비는 쪽 반대 포지션, n≈196(90일):

| 보유 | 합계손익 | t값 |
|---|---|---|
| 4h | +0.065% | 1.03 (무의미) |
| 8h | +0.256% | 2.71 |
| **16h** | **+0.461%** | **3.62** (다중비교 보정 후에도 통과) |
| 24h | +0.483% | 2.69 |

- **엣지의 출처는 캐리가 아니라 되돌림**(캐리 0.011% vs 방향손익 0.451%) → "펀딩 수취"가 아니라 "펀딩 극단 = 과열 신호"로 읽어야 한다
- 4h에선 안 나온다 → **현재 엔진의 8시간 최대보유 스캘핑 설계와 상충**
- 손절 없는 고정보유 수익률이고 승률 55%라 꼬리 위험이 열려 있다. n=196·단일 국면·16조합 탐색이므로 **유망 후보이지 확립된 전략이 아니다.** 실투자 전 추가 검증 필수
