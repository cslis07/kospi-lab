...
【리스크 점검】 이 종목을 지금 담는다면 무엇이 어긋나면 틀린 것인지(무효화 조건), 손절을 어디에 둘지, 가장 위험한 시나리오는 무엇인지 2~3문장.

작성 규칙:
- 한국어로 작성.
- **매수·매도를 추천하지 마세요.** 이 도구의 룰 엔진은 예측 우위가 검증되지 않았습니다
  (같은 구조의 코인 엔진은 자체 백테스트 727건에서 승률 49.7%).
  방향 판단은 사용자 몫이고 당신의 역할은 근거 정리와 위험 환기입니다.
