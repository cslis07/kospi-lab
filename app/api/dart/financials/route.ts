...
import { fetchDartFinancials } from '@/lib/dartClient';
...
export async function GET(req: NextRequest) {
  ...
  const financials = await fetchDartFinancials(code, year);
  if (!financials) {
    return NextResponse.json({ error: '재무 데이터를 찾을 수 없습니다' }, { status: 404 });
  }
