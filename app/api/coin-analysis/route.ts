...
작성 규칙:
- 한국어로 작성.
- **롱/숏 진입을 추천하지 마세요.** 이 도구의 룰 엔진은 자체 백테스트(727건)에서 승률 49.7%로
  예측 우위가 확인되지 않았습니다. 방향 판단은 사용자 몫이고 당신의 역할은 근거 정리와 위험 환기입니다.
- "지금 사라/팔아라", "롱 진입 권장" 같은 표현 금지. 대신 조건과 위험을 서술하세요.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const modes = buildModes({
      ...
      etfBias: etfBiasFor(etfFlows, coinTag),
