...
export default function StockDetailPage({
  ...
  const { data: stock, isLoading } = useSWR<StockData>(
    `/api/stock/${ticker}`,
    fetcher,
    { refreshInterval: 5000 }
  );
  const { data: chart } = useSWR<ChartPoint[]>(
    `/api/stock/${ticker}/chart?count=${tf.count}&tf=${tf.tf}`,
    fetcher,
  ...
  const [qty, setQty] = useState(String(myPortfolio?.quantity ?? ''));
  const [avg, setAvg] = useState(String(myPortfolio?.avgPrice ?? ''));
  const [alertAbove, setAlertAbove] = useState(String(myAlert?.above ?? ''));
  const [alertBelow, setAlertBelow] = useState(String(myAlert?.below ?? ''));
  ...
  const chartMin = chart ? Math.min(...chart.map((p) => p.price)) * 0.995 : 0;
  const chartMax = chart ? Math.max(...chart.map((p) => p.price)) * 1.005 : 0;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-6 mb-4">
        ...
        {chart && chart.length > 0 ? (
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={chart} margin={{ top: 5, right: 5, left: 10, bottom: 5 }}>
              <defs>
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div className="bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-xs">
                      <p className="text-gray-400">{d.date.replace(/(\d{4})(\d{2})(\d{2})/, '$1.$2.$3')}</p>
      ...
      {/* Portfolio & Alert in two columns */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
          ...
          {myPortfolio && stock && (
            <div className="mb-4 p-3 rounded-lg bg-white/5 text-xs space-y-1">
              ...
              <div className={`flex justify-between font-semibold ${(stock.price - myPortfolio.avgPrice) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                <span>손익</span>
                <span>{(stock.price - myPortfolio.avgPrice) >= 0 ? '+' : ''}{fmt(Math.round((stock.price - myPortfolio.avgPrice) * myPortfolio.quantity))}원</span>
              </div>
            </div>
          )}
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
          ...
          <div className="space-y-2">
            ...
            {myAlert && (
              <button
                onClick={() => { removeAlert(ticker); setAlertAbove(''); setAlertBelow(''); }}
